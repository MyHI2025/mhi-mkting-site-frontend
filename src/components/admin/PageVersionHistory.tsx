...
import type { PageVersion } from "@myhi2025/shared";
...
export function PageVersionHistory({ pageId }: PageVersionHistoryProps) {
  ...
  const [selectedVersion, setSelectedVersion] = useState<PageVersion | null>(
    null
  );
  ...
  const restoreMutation = useMutation({
    mutationFn: async (versionId: string) => {
      const response = await fetch(
        api.admin.pageVersionRestore(pageId, versionId),
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        const error = await response
          .json()
          .catch(() => ({ message: "Failed to restore version" }));
        throw new Error(
          error.message || error.error || "Failed to restore version"
        );
      }
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description:
          "The page has been successfully restored to the selected version.",
      });
    },
    onError: (error: Error) => {
      toast({
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {selectedVersion.isPublished ? "Yes" : "No"}
                  </p>
                </div>
              </div>

...
import { Mail, Phone, Calendar, Filter, Download, User, Building2, RefreshCw } from "lucide-react";
...
export default function AdminContactsPage() {
  ...
  const syncToZohoMutation = useMutation({
    ...
    onSuccess: (data: any) => {
      toast({
        ...
        description: data.message || `Synced ${data.success} contacts successfully`,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const filteredContacts = contacts.filter((contact) => {
    ...
    const matchesType = userTypeFilter === "all" || contact.userType === userTypeFilter;
  ...
  // Export to CSV
  const exportToCSV = () => {
    const headers = ["Title", "First Name", "Last Name", "Email", "Phone", "City", "Country", "User Type", "Organization", "Message", "Date"];
    const csvData = filteredContacts.map(contact => [
      ...
      contact.createdAt ? format(new Date(contact.createdAt), "yyyy-MM-dd HH:mm:ss") : "",
    ]);
  ...
  return (
    <div className="p-6 space-y-6" data-testid="admin-contacts-page">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold" data-testid="contacts-title">Contact Form Submissions</h1>
        ...
        <div className="flex gap-2">
          ...
          >
            <RefreshCw className={`mr-2 h-4 w-4 ${syncToZohoMutation.isPending ? 'animate-spin' : ''}`} />
            Sync to Zoho CRM
          </Button>
          <Button onClick={exportToCSV} disabled={filteredContacts.length === 0} data-testid="button-export-csv">
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {contacts.filter(c => {
                ...
                const contactDate = c.createdAt ? new Date(c.createdAt) : null;
                return contactDate && contactDate.toDateString() === today.toDateString();
              }).length}
            </div>
          </CardContent>
        </Card>
            ...
            <div className="space-y-2">
              ...
              <Select value={userTypeFilter} onValueChange={setUserTypeFilter}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="business">Corporates or Businesses</SelectItem>
      ...
      <Card>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-center py-8 text-destructive" data-testid="contacts-error-message">
              <p className="font-semibold">Failed to load contacts</p>
              <p className="text-sm mt-2">{error instanceof Error ? error.message : "An unexpected error occurred"}</p>
            </div>
            ...
            <div className="space-y-4">
              {filteredContacts.map((contact) => (
                <Card key={contact.id} className="hover:bg-muted/50 transition-colors" data-testid={`contact-card-${contact.id}`}>
                      ...
                      <div className="flex-1 space-y-3">
                        <div className="flex items-start justify-between">
                          <div className="flex items-center gap-3">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="font-semibold text-lg" data-testid={`contact-name-${contact.id}`}>
                                {contact.title ? `${contact.title} ` : ''}{contact.firstName} {contact.lastName}
                              </span>
                            </div>
                            <Badge 
                              className={`${USER_TYPE_COLORS[contact.userType] || 'bg-gray-500'} text-white`}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                          <div className="flex items-center gap-2 text-muted-foreground">
                            ...
                            <a href={`mailto:${contact.email}`} className="hover:underline" data-testid={`contact-email-${contact.id}`}>
                              {contact.email}
                            </a>
                          </div>
                          {contact.phone && (
                          ...
                          {contact.organization && (
                            <div className="flex items-center gap-2 text-muted-foreground">
                              ...
                              <span data-testid={`contact-organization-${contact.id}`}>{contact.organization}</span>
                            </div>
                          )}
                          {contact.country && (
                            <div className="flex items-center gap-2 text-muted-foreground">
                              <span className="text-base" data-testid={`contact-country-${contact.id}`}>🌍 {contact.country}</span>
                            </div>
                          )}
                        </div>
                        ...
                        <div className="pt-2">
                          ...
                          <p className="text-sm text-muted-foreground whitespace-pre-wrap" data-testid={`contact-message-${contact.id}`}>
                            {contact.message}
                          </p>
                        </div>
                      </div>

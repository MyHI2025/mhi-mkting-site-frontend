...
import type { Page, ContentSection, ContentBlock } from "@myhi2025/shared";
...
export default function AdminContentPage() {
  ...
  const [selectedPageForContent, setSelectedPageForContent] =
  ...
  const [sectionToDelete, setSectionToDelete] = useState<ContentSection | null>(
    null
  );

  // Media library state
  const [isMediaLibraryOpen, setIsMediaLibraryOpen] = useState(false);
  const [currentImageField, setCurrentImageField] = useState<"imageUrl" | null>(
    null
  );
  ...
  const { data: contentSections = [], isLoading: sectionsLoading } = useQuery<
    ContentSection[]
  >({
    queryKey: [api.admin.pages, selectedPageForContent?.id, "sections"],
    queryFn: () =>
      selectedPageForContent
        ? apiRequest(api.admin.pageSections(selectedPageForContent.id))
        : [],
  ...
  const createPageMutation = useMutation({
    mutationFn: (data: CreatePageForm) =>
      apiRequest(api.admin.pages, "POST", data),
    onSuccess: () => {
  ...
  const migratePageMutation = useMutation({
    mutationFn: (pageId: string) =>
      apiRequest(api.admin.pageMigrate(pageId), "POST"),
    onSuccess: (data) => {
    ...
    onError: (error: any) => {
      const isAlreadyMigrated = error.message?.includes(
        "already has content sections"
      );
      toast({
  ...
  const deletePageMutation = useMutation({
    mutationFn: (pageId: string) =>
      apiRequest(api.admin.page(pageId), "DELETE"),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [api.admin.pages] });
      toast({
        title: "Page deleted",
  ...
  const createSection = async (sectionType: string) => {
    ...
    try {
      await apiRequest(api.admin.sections, "POST", {
        ...
        title:
          getSectionTypes().find((s) => s.type === sectionType)?.label ||
          sectionType,
        subtitle: "",
        content: {},
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-2">
            Manage your website pages and content. Create, edit, and publish
        ...
        <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
          ...
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>
                Add a new page to your website. Pages are used to organize
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Update the page information. Changes will be saved to your
              website.
            </DialogDescription>
          </DialogHeader>
          <Form {...editForm}>
      ...
      <Dialog
        open={isContentSectionsOpen}
        onOpenChange={setIsContentSectionsOpen}
      >
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Manage the content sections and blocks for this page. Add, edit,
              and reorder sections to build your page content.
            </DialogDescription>
          </DialogHeader>
                ...
                <CardContent>
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-gray-500 dark:text-gray-400 mt-1">
                        This page doesn't have editable content sections yet.
                        You can migrate it to create sections based on existing
                        content, or start fresh.
                      </p>
                    </div>
                      ...
                      <Button
                        onClick={() => {
                          if (selectedPageForContent?.id) {
                            migratePageMutation.mutate(
                              selectedPageForContent.id
                            );
                          }
                        }}
                        disabled={
      ...
      <Dialog open={isCreateSectionOpen} onOpenChange={setIsCreateSectionOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Choose a section type to add to your page. Each section type has
      ...
      <Dialog open={isEditSectionOpen} onOpenChange={setIsEditSectionOpen}>
        <DialogContent className="sm:max-w-4xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Update the section information and rich content using the WYSIWYG
              editor below.
            </DialogDescription>
          </DialogHeader>
          <Form {...editSectionForm}>
            <form
              onSubmit={editSectionForm.handleSubmit(updateSection)}
              className="space-y-6"
            >
              ...
              <div className="flex justify-end space-x-2 pt-4">
                ...
                <Button type="submit" data-testid="button-submit-edit-section">
                  Update Section
                </Button>
              </div>
            </form>
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {sectionToDelete?.title || sectionToDelete?.sectionType}"? This
              will also delete all content blocks within this section. This
              action cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
